...
// Clean up any stale/unclean customerName or mobile values loaded from file
accounts = accounts.map((acc) => ({
  ...acc,
  customerName: cleanExtractedValue(acc.customerName, 'name'),
  mobile: cleanExtractedValue(acc.mobile, 'mobile'),
  expiryDate: cleanExtractedValue(acc.expiryDate, 'date'),
}));
saveAccounts();

function cleanExtractedValue(text: string | null | undefined, type: 'name' | 'mobile' | 'date'): string | null {
  if (!text) return null;
  ...
  const noiseKeywords = [
    'dashboard', 'pay bill', 'payment history', 'ticket', 'report', 'logout',
    'account status', 'connection status', 'expiry date', 'plan rate', 'recharge',
    'bkash', 'trxid', 'cash', 'user name', 'mobile', 'phone', 'login', 'customer',
    'online', 'offline'
  ];
  ...
  if (noiseCount >= 2 || clean.length > 60) {
    return null;
  }

  if (type === 'name') {
    if (clean.length > 35 || lower.startsWith('user name') || lower.startsWith('customer name')) return null;
    return clean;
  }

  if (type === 'mobile') {
    const phoneMatch = clean.match(/(01[3-9]\d{8}|\+?8801[3-9]\d{8})/);
    if (phoneMatch) {
      return phoneMatch[0];
    }
    const digitsOnly = clean.replace(/[^\d]/g, '');
    if (digitsOnly.length >= 10 && digitsOnly.length <= 14 && clean.length <= 20) {
      return clean;
    }
    return null;
  }
...
async function checkAccountConnection(account: StoredAccount): Promise<{
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Extract Account Status, Expiry Date, Customer User Name, and Mobile from Dashboard HTML
      let accountStatus: 'Active' | 'Expired' | 'Unknown' = 'Unknown';
      let expiryDate: string | null = null;
      let customerName: string | null = null;
      let mobile: string | null = null;

      // 1. Direct search on <li> items (e.g. <li class="flex..."><p>User Name:</p><span>mohammad.s</span></li>)
      $resp('li').each((_, el) => {
        const $li = $resp(el);
        const pText = $li.children('p').text().trim().toLowerCase();
        const spanText = $li.children('span').text().trim();

        if (pText.includes('user name') || pText.includes('customer name')) {
          const c = cleanExtractedValue(spanText, 'name');
          if (c) customerName = c;
        }
        if (pText.includes('mobile') || pText.includes('phone')) {
          const c = cleanExtractedValue(spanText, 'mobile');
          if (c) mobile = c;
        }
      });

      // 2. Targeted search for specific labels on small/leaf elements
      $resp('p, span, td, th').each((_, el) => {
        const $el = $resp(el);
        if ($el.children().length === 0) {
          const raw = $el.text().trim();
          const lower = raw.toLowerCase();

          if (!customerName && (lower === 'user name:' || lower === 'user name' || lower === 'customer name:')) {
            const val = $el.siblings('span, p, td').first().text().trim() || $el.next().text().trim();
            const c = cleanExtractedValue(val, 'name');
            if (c) customerName = c;
          }

          if (!mobile && (lower === 'mobile:' || lower === 'mobile' || lower === 'mobile no:' || lower === 'phone:')) {
            const val = $el.siblings('span, p, td').first().text().trim() || $el.next().text().trim();
            const c = cleanExtractedValue(val, 'mobile');
            if (c) mobile = c;
          }

          if (lower === 'account status' || lower === 'account status:') {
            const val = $el.siblings('span, p, font, td').first().text().trim() || $el.next().text().trim();
            if (val) {
              const u = val.toUpperCase();
              if (u.includes('ACTIVE')) accountStatus = 'Active';
              else if (u.includes('EXPIRED') || u.includes('INACTIVE')) accountStatus = 'Expired';
            }
          }

          if (!expiryDate && (lower === 'expiry date' || lower === 'expiry date:')) {
            const val = $el.siblings('span, p, td').first().text().trim() || $el.next().text().trim();
            const c = cleanExtractedValue(val, 'date');
            if (c) expiryDate = c;
          }
        }
      });

      // 3. Precise Regex Fallbacks on raw HTML
      if (!customerName) {
        const nameMatches = [
          dashboardHtml.match(/User\s*Name\s*:?\s*<\/p>\s*<span[^>]*>([a-zA-Z0-9_.\s\-]{2,35})<\/span>/i),
          dashboardHtml.match(/User\s*Name\s*:?\s*<\/span>\s*<span[^>]*>([a-zA-Z0-9_.\s\-]{2,35})<\/span>/i),
        ];
        for (const m of nameMatches) {
          if (m && m[1]) {
            const c = cleanExtractedValue(m[1], 'name');
            if (c) {
              customerName = c;
              break;
            }
      ...
      if (!mobile) {
        const mobMatches = [
          dashboardHtml.match(/Mobile\s*:?\s*<\/p>\s*<span[^>]*>([\d\s+\-]{7,16})<\/span>/i),
          dashboardHtml.match(/Mobile\s*:?\s*<\/span>\s*<span[^>]*>([\d\s+\-]{7,16})<\/span>/i),
          dashboardHtml.match(/(01[3-9]\d{8})/),
        ];
        for (const m of mobMatches) {
          if (m && (m[1] || m[0])) {
            const c = cleanExtractedValue(m[1] || m[0], 'mobile');
            if (c) {
              mobile = c;
              break;
            }
      ...
      if (accountStatus === 'Unknown') {
        if (upperHtml.includes('ACTIVE</FONT>') || upperHtml.includes('>ACTIVE<') || upperHtml.includes('COLOR="GREEN">ACTIVE')) {
          accountStatus = 'Active';
        } else if (upperHtml.includes('EXPIRED</FONT>') || upperHtml.includes('>EXPIRED<') || upperHtml.includes('COLOR="RED">EXPIRED')) {
          accountStatus = 'Expired';
        }
      }

      if (!expiryDate) {
        const dateMatch = dashboardHtml.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},\s+\d{4}/i) ||
                          dashboardHtml.match(/\d{4}-\d{2}-\d{2}/) ||
                          dashboardHtml.match(/\d{1,2}[-\/]\d{1,2}[-\/]\d{4}/);
        if (dateMatch) {
...
async function runScan() {
  ...
  async function worker() {
    while (queue.length > 0 && !shouldStop) {
      ...
      if (result.accountStatus) acc.accountStatus = result.accountStatus;
      if (result.expiryDate !== undefined) acc.expiryDate = result.expiryDate;
      if (result.customerName !== undefined) acc.customerName = result.customerName;
      if (result.mobile !== undefined) acc.mobile = result.mobile;
...
app.get('/api/accounts', (req, res) => {
  ...
  if (status && status !== 'ALL') {
    ...
    if (uppercaseStatus === 'ACTIVE') {
      filtered = filtered.filter((a) => a.accountStatus === 'Active');
    } else if (uppercaseStatus === 'EXPIRED') {
      filtered = filtered.filter((a) => a.accountStatus === 'Expired');
    } else {
      filtered = filtered.filter((a) => a.status === uppercaseStatus);
    }
  }
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (a) =>
        ...
        (a.mobile && a.mobile.toLowerCase().includes(term)) ||
        (a.accountStatus && a.accountStatus.toLowerCase().includes(term)) ||
        (a.expiryDate && a.expiryDate.toLowerCase().includes(term))
    );
  }
  ...
  if (sortBy) {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === 'mobile') {
        return (a.mobile || '').localeCompare(b.mobile || '') * order;
      }
      if (sortBy === 'status') {
        return a.status.localeCompare(b.status) * order;
      }
      if (sortBy === 'accountStatus') {
        return (a.accountStatus || 'Unknown').localeCompare(b.accountStatus || 'Unknown') * order;
      }
...
app.get('/api/summary', (req, res) => {
  ...
  const expired = accounts.filter((a) => a.accountStatus === 'Expired').length;
...
app.delete('/api/accounts', (req, res) => {
  const { id, ids, clearAll } = req.query;
  ...
  if (Array.isArray(bodyIds)) {
    bodyIds.forEach((i) => targetIdsSet.add(String(i)));
  }
  if (ids) {
    String(ids).split(',').forEach((i) => targetIdsSet.add(i.trim()));
  }
  if (id) {
    targetIdsSet.add(String(id));
  }

  if (targetIdsSet.size > 0) {
    const initialCount = accounts.length;
    accounts = accounts.filter((a) => !targetIdsSet.has(a.id));
    saveAccounts();
    addLog({
      username: 'SYSTEM',
      ...
      status: 'SYSTEM',
      success: true,
      reason: `Deleted ${initialCount - accounts.length} account(s)`,
    });
    return res.json({ message: `Deleted ${initialCount - accounts.length} account(s)` });
  }
...
// Export Results
app.all('/api/export', (req, res) => {
  const format = req.query.format || req.body?.format || 'csv';
  const rawIds = req.query.ids || req.body?.ids;
  const excludeStatus = req.query.excludeStatus || req.body?.excludeStatus;
  ...
  if (rawIds) {
    const idList = Array.isArray(rawIds)
      ...
      : String(rawIds)
          ...
          .filter(Boolean);
    if (idList.length > 0) {
      targetAccounts = targetAccounts.filter((a) => idList.includes(a.id));
    }
  }

  if (excludeStatus) {
    const statusesToExclude = String(excludeStatus)
      .toUpperCase()
      .split(',')
      .map((s) => s.trim());
    targetAccounts = targetAccounts.filter((a) => !statusesToExclude.includes(a.status));
  }
  ...
  if (format === 'json') {
    ...
    return res.send(
      JSON.stringify(
        targetAccounts.map(({ password, ...rest }) => rest),
        null,
        2
      )
    );
  ...
  // Default CSV format
  let csvContent = 'Login Username,Customer Name,Mobile No.,Connection Status,Account Status,Expiry Date,Last Checked,Response Time (ms),Error\n';
  for (const acc of targetAccounts) {
    csvContent += `"${acc.username}","${acc.customerName || ''}","${acc.mobile || ''}","${acc.status}","${acc.accountStatus || 'Unknown'}","${acc.expiryDate || 'N/A'}","${
      acc.lastChecked || 'N/A'
    }","${acc.responseTimeMs || 'N/A'}","${acc.error || ''}"\n`;
  }
...
app.post('/api/accounts/check-single', async (req, res) => {
  ...
  if (existingIndex !== -1) {
    ...
    if (result.accountStatus) accounts[existingIndex].accountStatus = result.accountStatus;
    if (result.expiryDate !== undefined) accounts[existingIndex].expiryDate = result.expiryDate;
    if (result.customerName !== undefined) accounts[existingIndex].customerName = result.customerName;
    if (result.mobile !== undefined) accounts[existingIndex].mobile = result.mobile;
